...
export function PlayingCard({
  ...
  const symbol = card.isJoker ? '★' : SUIT_SYMBOL[card.suit as Suit];
  const label = card.isJoker ? 'JO' : card.rank;
  ...
  return (
    <motion.button
      ...
      className={`card-face ${size} relative rounded-lg select-none cursor-pointer
        border ${selected ? 'border-gold-500 ring-2 ring-gold-400' : 'border-black/10'}
        ${selected ? 'shadow-card-hover' : 'shadow-card'} ${colorClass}`}
      {...draggableProps}
    >
